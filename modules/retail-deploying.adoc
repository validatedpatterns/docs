...
.. Optional: Edit the `values-global.yaml` file to customize the deployment for your cluster. For a description of each field, see link:/learn/values-global-configuration/[Global configuration reference].
+
The defaults should suffice if you want to see the apps running. The values that you might change are under the `imageregistry`, if you copied the images to your own quay account and hostname. You can also change the `git` settings of `account`, `email` and `hostname` to reflect your own account settings.
+
...
.. If you made changes, stage and commit them by running the following commands:
+
[source,terminal]
----
