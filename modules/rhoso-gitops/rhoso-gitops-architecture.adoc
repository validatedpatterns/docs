...
Both the parent Application and child Applications run in `vp-gitops` and sync
upstream Kustomize overlays. All child Applications deploy at sync-wave 0 and
converge eventually through retry policies.
