...
build: ## Build the website locally in the public/ folder
	podman run -it --pull=newer --net=host -v $(PWD):/site:$(ATTRS) --entrypoint hugo $(HOMEPAGE_CONTAINER)

.PHONY: serve
serve: ## Build the website locally from a container and serve it
	@echo "Serving via container. Browse to http://localhost:4000"
	podman run -it --pull=newer --net=host -v $(PWD):/site:$(ATTRS) --entrypoint hugo $(HOMEPAGE_CONTAINER) server -p 4000

.PHONY: htmltest
htmltest: build ## Runs htmltest against the site to find broken links
	@echo "Running html proof to check links"
	podman run -it --pull=newer --net=host -v $(PWD):/site:$(ATTRS) --entrypoint htmltest $(HOMEPAGE_CONTAINER)

.PHONY: run
run: ## Runs the container interactively
	@echo "Running html proof to check links"
	podman run -it --pull=newer --net=host -v $(PWD):/site:$(ATTRS) $(HOMEPAGE_CONTAINER)
...
spellcheck: ## Runs a spellchecker on the content/ folder
	...
	podman run -it --pull=newer -v $(PWD):/tmp:$(ATTRS) docker.io/jonasbn/github-action-spellcheck:latest
