---
title: Medical Diagnosis Demo Script
weight: 40
...
Technologies Used:
The medical diagnosis pattern utilizes technologies from Red Hat and from OpenSource Projects.. 

At the core of the pattern is OpenShift GitOps (ArgoCD) providing a gitops controller to enforce the desired state of our applications deployed to our clusters. This pattern also utilizes technologies like Red Hat AMQ-Streams for Kafka, Red Hat OpenShift Serverless for Knative Eventing and Knative Serving functions. The model storage is provided by Red Hat OpenShift Data Foundations which provides a cloud-native solution for block, file and object storage requirements. 

Secrets management is built-in to all Validated Patterns. Currently the backend secrets manager is HashiCorp Vault. In addition to vault we use the External Secrets Operator to abstract the creation of kubernetes secrets from the backend providers. The external secrets operator also allows us to have flexibility with different back end providers. 

Medical Diagnosis uses Grafana dashboards to visualize the pipeline and pipeline metrics.


OpenShift Console and Medical Diagnosis Dashboard
Login to your cluster using `kubeadmin`, then click operators > installed operators in the left hand menu.

We can see that we have a number of operators already installed on our cluster. This was achieved using the validated pattern framework for medical diagnosis. Keep in mind that these aren’t just the operators being installed, it’s the operators, an instance of the operator and the integration between the various operators that this application needs to run.

Next, let’s switch to the developer context. Search for xraylab, then click topologies. When we zoom out we can see the individual resources that are created to make up this application. Let’s start the demonstration by scaling the image-generator deploymentConfig up to 1. We’ll discuss why we’re doing this in just a moment.
