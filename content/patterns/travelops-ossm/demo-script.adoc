...
== Introduction
The travelops pattern is a complete deployment of {rh-sm}, including {rh-elastic}, {rh-kiali}, and {rh-jaeger}. Together these technologies provide a means
to view, allow and deny network traffic between services in your cluster. This pattern is a combination of the travel portal tutorial provided
and the path to production demo provided by members of the Red Hat Solution Patterns team.
