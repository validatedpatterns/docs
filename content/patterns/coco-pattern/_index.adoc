...
date: 2024-10-03
tier: sandbox
summary: Deploy confidential containers on OpenShift using hardware-backed Trusted Execution Environments (TEEs) on Azure and bare metal with Intel TDX, AMD SEV-SNP, and NVIDIA H100 confidential GPU support.
rh_products:
...
**Cloud deployments** use "peer pods" — confidential VMs from the Azure `Standard_DCas_v5` family provisioned directly on the Azure hypervisor rather than nested inside OpenShift worker nodes.

**Bare metal deployments** support Intel TDX (Trusted Domain Extensions) and AMD SEV-SNP (Secure Encrypted Virtualization - Secure Nested Paging) hardware TEEs, with optional NVIDIA H100 confidential GPU support for protected GPU workloads.
...
- **`baremetal-gpu`** — Bare metal with NVIDIA H100 confidential GPU support
...
- For GPU topology: NVIDIA H100 GPUs with confidential computing firmware
- Tools: `podman`, `yq`, `jq`, `skopeo`
- An OpenShift pull secret at `~/pull-secret.json`
...
* Extending bare metal GPU support to additional GPU models and configurations
...
- **Intel DCAP** _(bare metal with TDX)_: Provisioning Certificate Caching Service (PCCS) and Quote Generation Service (QGS) for Intel TDX remote attestation via the Intel PCS API.
- **NVIDIA GPU Operator** _(GPU topology only)_: Manages NVIDIA H100 confidential GPUs with CC Manager, VFIO passthrough, and Kata device plugins for GPU-enabled confidential workloads.
