...
The Edge GitOps VMs Helm chart provides a flexible and scalable way to define and deploy Virtual Machines (VMs) using OpenShift Virtualization. At its core, the chart leverages a https://github.com/validatedpatterns/ansible-edge-gitops/blob/main/charts/hub/edge-gitops-vms/templates/virtual-machines.yaml[template file]  that allows for dynamic configuration of multiple VM groups. Key features include: 

* Defining one or more VM groups (e.g., kiosk) with a configurable number of instances per group.
* Customizing compute resources (CPU, memory, storage) for each group.
* Specifying SSH key credentials for secure access per group.
* Selecting different operating systems for each VM group.
* Configuring TCP/UDP ports to be exposed per group.

This modular approach abstracts the complexities of VM provisioning in OpenShift Virtualization, enabling users to focus on defining the types and quantities of VMs needed. The chart supports highly scalable deployments, making it suitable for environments with large compute resources, such as AWS metal nodes with 72 cores and 192 GB RAM.
...
Additionally, you can copy and customize these templates if you wish.
The https://github.com/validatedpatterns/ansible-edge-gitops/blob/main/charts/hub/edge-gitops-vms/templates/rhel8-kiosk-with-svc.yaml[template
...
Follow these steps to create a VM from the console using a template:

. Access the Virtualization Tab

.. Navigate to the OpenShift Console.

.. Click on the *Virtualization* tab.

.. Locate the list of available *VM templates*.

.. Identify the *Create VM button* next to the desired template.
+
...
.. The VM Creation Wizard will open, displaying configuration options.
+
...
.. Accept the default settings or modify them as needed

.. Click Create to proceed.

.. After successful creation, a confirmation screen will appear
+
...
. Monitor the VM Lifecycle

.. Navigate to the *VirtualMachines* tab.

.. Observe the VM’s status as it spins up.

.. The VM will remain active until manually deleted
+
...
The initial user (default: `cloud-user`) and initial password are customizable through values overrides. The `kiosk` type shows
...
`kiosk-ssh`, but could support more, such as `iot-ssh`, `gateway-ssh`, and so on more details on how to expand on this pattern are described below.
...
Access the virtual machine console using the OpenShift Console by following this procedure:
...
[NOTE]
====
In OpenShift Virtualization 4.11, the `Open Console` option appears when you click the virtual machine name in OpenShift console.
