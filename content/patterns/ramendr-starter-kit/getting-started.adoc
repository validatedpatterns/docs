...
* **Metal workers for VMs** — For the `odf` variant, both regional clusters need at least one worker node with a metal instance type (for example `m5.metal`) to run the sample VM workload.
