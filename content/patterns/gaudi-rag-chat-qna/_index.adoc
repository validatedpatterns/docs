...
* Deploy all required AI Chat microservices:
...
** Backend service
** Frontend service
* Deploy TGI (Text Generation Inference) using Red Hat OpenShift AI
...
* OPEA QnA Chat stack consists of: Embedding service, Vector Database, Retriever, Reranking service, data preparation service, LLM server and back-/frontend applications.

* Gaudi enablement stack consists of: Node Feature Discovery, Kernel Module Management and HabanaAI operators.

* Additionaly validated pattern makes use of OpenShift Data Foundation operator to host S3-like storage, Image Registry to store all built images and Hashicorp Vault to safely keep HuggingFace User token.
