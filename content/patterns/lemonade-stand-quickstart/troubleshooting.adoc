...
. If applications are stuck in `Progressing`, wait an additional 10 minutes and re-run the health check. Detector model downloads from Hugging Face via MinIO and GPU operator initialization can take significant time.
...
The Guardrails Orchestrator depends on all detector models being available and healthy before it can start serving requests.
