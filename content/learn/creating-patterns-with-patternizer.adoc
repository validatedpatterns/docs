...
Patternizer does not currently generate `pattern-metadata.yaml`.
Add this file manually at the repository root after initialization, or copy it from an existing pattern such as link:https://github.com/validatedpatterns/multicloud-gitops/blob/main/pattern-metadata.yaml[multicloud-gitops].
For more information about metadata fields, see link:/contribute/pattern-metadata-schema-reference/[Schema and field reference].
For the documentation sync workflow, see link:/contribute/pattern-metadata-documentation-sync/[Metadata and documentation].
...
* Add link:/contribute/pattern-metadata/[Pattern metadata] so cluster sizing and CI information stay in sync with the documentation site
