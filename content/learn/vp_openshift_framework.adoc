...
The `pattern-metadata.yaml` file at the repository root defines pattern identity, tier, links, and cluster sizing.
The documentation site syncs this file into cluster sizing pages automatically.
For more information about metadata fields, see link:/contribute/pattern-metadata/[Pattern metadata].
...
Both examples include `pattern-metadata.yaml` at the repository root.
