...
Keep `patternRepo` in the testing artifact JSON aligned with `repo_url` in the `pattern-metadata.yaml` file for the pattern repository.
The `ci_url` field in the metadata file must point to the same pattern entry on the CI dashboard.
