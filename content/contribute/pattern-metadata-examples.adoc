...
external_requirements:
  cluster_sizing_note: >
    This pattern requires nodes with GPUs that are only supported in
...
description: A multi-cloud pattern that uses GitOps, HashiCorp Vault and ACM to manage applications and secrets across clusters.
